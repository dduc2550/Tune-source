...
class SongController extends Controller
{
    ...
    {
        $allSong = DB::table('songs')->where('deleted_at',null)->get();
        return view("frontend.home.listen",['songs'=>$allSong]);
    }
    ...
    public function handleCreate(StorePostSongRequest $request){
        ...
        if($request->hasFile('poster_music')){
            ...
            if($request->file('poster_music')->isValid()){
                //file khong co loi
                $upload = $request->file('poster_music')->move('uploads/images',$namePoster);
            }
        }

        $insert = DB::table('songs')->insert([
    ...
    public function delete(Request $request)
    {
        $idSong = $request->id;
        $idSong = is_numeric($idSong) ? $idSong : 0;
        $del = DB::table('songs')->where('id',$idSong)->update(['deleted_at' => date('Y-m-d H:i:s')]);
        if($del){
